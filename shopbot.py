...
from db_operations import load_countries, load_products, load_qrcode_url, load_thank_you_message
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value=(
            "ใช้คำสั่ง `!สั่งของ` แล้วตามด้วยรายการสินค้าที่ต้องการในรูปแบบ:<ประเทศ> <ชื่อสินค้า> <จำนวน>\n\n"
            "ตัวอย่าง:\n"
            "```!สั่งของ\n"
            "1 ชุดเกาะมวยไทย 5\n"
            "1 ผ้าพันมือมวยไทย 2\n"
            "2 ชุดกิโมโน 3```\n\n"
            "• <ประเทศ> ใช้ตัวเลข 1-5 แทนประเทศ"
        ),
        inline=False
    )
...
async def order_many_command(ctx):
    ...
    if len(command_parts) < 2:
        ...
        usage_embed.add_field(
            name="รูปแบบ",
            value=(
                "แต่ละบรรทัดใช้รูปแบบ: `<ประเทศ> <ชื่อสินค้า> <จำนวน>`\n\n"
                "• <ประเทศ> ใช้ตัวเลข 1-5 แทนประเทศ\n"
    ...
    for line in order_lines:
        ...
        if len(parts) < 3:
            failed_orders.append((line, "รูปแบบไม่ถูกต้อง ต้องเป็น <ประเทศ> <ชื่อสินค้า> <จำนวน>"))
            continue
        
        country = parts[0]
        if not country.isdigit() or int(country) < 1 or int(country) > 5:
            failed_orders.append((line, f"รหัสประเทศไม่ถูกต้อง: {country} (ต้องเป็นตัวเลข 1-5)"))
            continue
        ...
        if not product:
            failed_orders.append((line, f"ไม่พบสินค้า: {product_name} ในประเทศ {country}"))
            continue
        ...
        successful_orders.append((
            f"{COUNTRY_EMOJIS.get(country, '')} {product['emoji']} {product['name']} x{quantity} = {item_price:,.2f} บาท"
        ))
